...
// Tính toán interval sao cho chúng là bội số của 10
  ...
  List<Widget> _buildBarValueLabels(BoxConstraints constraints) {
    final chartWidth = constraints.maxWidth;
    final chartHeight = constraints.maxHeight;
    final barCount = data.length;
    final groupSpacing = 8.0;
    final barWidth = 40.0;
    final groupWidth = barWidth * 2 + groupSpacing;
    ...
    for (int i = 0; i < barCount; i++) {
      ...
      // Tính vị trí theo % chiều cao dựa vào maxY
      final actualTop = chartHeight * (1 - item.actual / maxY);
      final targetTop = chartHeight * (1 - item.target / maxY);

      // Label actual (cột đầu tiên trong nhóm)
      labels.add(Positioned(
        left: groupX + 4, // canh lề cho actual
        top: actualTop - 24,
        child: Text(
          item.actual.toStringAsFixed(1),
          style: TextStyle(
            fontSize: 12,
      ...
      // Label target (cột thứ hai trong nhóm)
      labels.add(Positioned(
        left: groupX + barWidth + 8, // canh lề cho target
        top: targetTop - 24,
        child: Text(
          item.target.toStringAsFixed(1),
          style: TextStyle(
            fontSize: 12,
            fontWeight: FontWeight.bold,
